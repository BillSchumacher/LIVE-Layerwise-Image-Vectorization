...
class OptimizableSvg:
    ...
    def reportSkippedAttribs(self, node, non_skipped=[]):
        ...
        if len(skipped)>0:
            ...
            print(
                self.offset_str(
                    f"""Warning: Skipping the following attributes of node '{tag}': {", ".join([f"'{atr}'" for atr in skipped])}"""
                )
            )
    ...
    def reportSkippedChildren(self,node,skipped):
        ...
        if len(skipped)>0:
            ...
            print(
                self.offset_str(
                    f"""Warning: Skipping the following children of node '{tag}': {", ".join([f"'{name}'" for name in skipped_names])}"""
                )
            )
